// sync-patterns.mjs — 將 UI_PATTERNS.md 同步到 Stitch，生成全 UI Patterns 展示頁
...
D) ToolbarAction (text-only): text #004680, SemiBold, no border — "檢視", "提交"
E) Danger delete: 36×36px, rounded-6, hover red bg — show ✕ icon
F) Ghost add: full-width dashed border, hover blue border — "+ 新增篩選條件"

═══════════════════════════════════════
SECTION 4: INPUT PATTERNS (D–F)
═══════════════════════════════════════
D) Standard Input: h-36px rounded-6px border rgba(145,158,171,0.2), focus border #1D7BF5
E) Floating Label Dropdown (DropdownSelect): h-54px rounded-8px with "狀態" label floating at top-left edge
...
H) Dropdown List (Export style):
  ...
  - Show: "匯出 Excel" (icon green) + "匯出 CSV" (icon blue)
...
═══════════════════════════════════════
J) Toast: fixed bottom, bg #1c252e, text white, rounded-8, shadow — "✓ 下載成功"
