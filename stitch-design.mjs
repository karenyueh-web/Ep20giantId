// stitch-design.mjs — 生成 Giant Global EP 設計系統規範頁面
...
5. SELECTION TOOLBAR — h-48px, bg #d9e8f5:
   ...
   - Text-only action buttons: "檢視" "全部提交廠商" in #004680, no background
...
try {
  ...
  try {
    ...
    console.log('⚠️  list_projects failed, creating new one...');
  }
  ...
  console.log('⏳ Generating design system page (this may take 30–60s)...');
  ...
  try {
    ...
    if (image) {
      ...
      console.log('🖼️  Preview → stitch-design-preview.png');
    }
  } catch (_) {
    console.log('⚠️  Image export not available for this screen.');
  }
