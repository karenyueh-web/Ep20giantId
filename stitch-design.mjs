// stitch-design.mjs ???Ÿæ? Giant Global EP è¨­è?ç³»çµ±è¦ç??é¢
...
5. SELECTION TOOLBAR ??h-48px, bg #d9e8f5:
   ...
   - Text-only action buttons: "æª¢è?" "?¨éƒ¨?äº¤å» å?" in #004680, no background
...
console.log('?Ž¨ Connecting to Stitch...');
...
try {
  ...
  try {
    ...
    console.log('? ï?  list_projects failed, creating new one...');
  }
  ...
  try {
    ...
    if (image) {
      ...
      console.log('?–¼ï¸? Preview ??stitch-design-preview.png');
    }
  } catch (_) {
    console.log('? ï?  Image export not available for this screen.');
  }
