...
const EXPORT_OPTIONS: ExportOption[] = [
  ...
  { type: 'batchCorrection',    label: '下載批次建立修正單',          icon: <FilePlus2 size={15} className="text-[#00a76f]" />, description: 'CK/B 訂單，填寫修正內容後上傳' },
];
...
export function exportBatchCorrectionTemplate(orders: OrderRow[], filename?: string) {
  ...
  const defaultName = filename || `批次建立修正單_${new Date().toISOString().slice(0, 10)}.xlsx`;
  buildXlsx('批次修正單', [BATCH_CORRECTION_HEADERS, ...instructionRows, ...rows], defaultName);
