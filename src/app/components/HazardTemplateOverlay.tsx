...
function MiniUploadZone({ file, onFileChange }: {
  ...
  return (
    <div
      className={`border-2 border-dashed rounded-[10px] flex flex-col items-center justify-center gap-[6px] py-[20px] cursor-pointer transition-colors ${
...
function FloatingInput({ label, value, onChange }: {
  ...
  return (
    <div className="relative w-full" style={{ minHeight: '46px' }}>
      <div className="absolute inset-0 pointer-events-none rounded-[8px] border border-[rgba(145,158,171,0.2)]" />
      <div className="absolute flex items-center left-[12px] px-[2px] top-[-5px] z-10">
        <div className="absolute bg-white h-[2px] left-0 right-0 top-[5px]" />
        <p className="text-[11px] font-semibold text-[#637381]">{label}</p>
      </div>
      <input
        className="w-full rounded-[8px] px-[12px] pt-[15px] pb-[8px] text-[13px] text-[#1c252e] outline-none bg-transparent border-0 leading-[20px]"
        value={value}
        ...
        <div className="flex-1 flex flex-col px-[40px] pt-[52px] pb-[32px] overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center gap-[8px] mb-[24px]">
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              ...
              <path d="M12 16v-4M12 8h.01" stroke="#1677ff" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          </div>
          ...
          <div className="flex flex-col gap-[16px]">
            {templates.map((t, i) => (
              <div key={t.id} className="flex items-center gap-[12px]">
                {/* 序號 */}
                <span className="font-['Public_Sans:Regular',sans-serif] text-[14px] text-[#637381] shrink-0 w-[16px] text-right">
                  {i + 1}
                </span>
                {/* 範本名稱 */}
                <span className="font-['Public_Sans:Regular',sans-serif] text-[14px] text-[#1c252e] shrink-0 min-w-[140px]">
                  {t.name}
                </span>
                {/* 檔案連結 */}
                <div className="flex items-center gap-[6px] flex-1 min-w-0">
                  ...
                  <span className="font-['Public_Sans:Regular',sans-serif] text-[13px] text-[#1677ff] underline truncate cursor-pointer hover:text-[#0958d9] transition-colors">
                    {t.fileName}
                  </span>
                </div>
                {/* 巨大才有編輯/刪除 */}
                {isGiant && (
                  <div className="flex items-center gap-[8px] shrink-0">
                    ...
                    <button onClick={() => handleEditOpen(t)}
                      className="hover:opacity-70 transition-opacity">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                        <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" stroke="#1890FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                        <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" stroke="#1890FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </button>
                  </div>
                )}
              ...
              <div className="flex flex-col px-[28px] pt-[52px] pb-[24px] gap-[16px] h-full">
                ...
                <div className="flex flex-col gap-[4px]">
                  ...
                  <div className="flex items-center gap-[6px]">
                    ...
                    <span className="text-[13px] text-[#1677ff] underline truncate">{editTarget.fileName}</span>
                  </div>
                </div>
                {/* 重新上傳 */}
