...
import {
  ...
  revertSampleOrderMdo,
  updateDraftSampleOrderMdo,
...
function utcToLocalDisplay(isoString: string | null | undefined): string {
  ...
  const handleSaveDraft = async () => {
    ...
    // 先呼叫 MDO update-draft，成功後才更新本地 store
    if (typeof order.id === 'string' && order.id.includes('-')) {
      try {
        const latest = await fetchSampleOrder(order.id);
        const revisionNo = latest?.mdoRevisionNo ?? order.mdoRevisionNo ?? 1;
        await updateDraftSampleOrderMdo({
          id: order.id,
          revisionNo,
          sampleType:  drSampleType,
          demandDate:  drDemandDate ? `${drDemandDate}T00:00:00.000Z` : undefined,
        ...
        const e = err as { body?: { error?: { message?: string } } };
        const msg = e?.body?.error?.message ?? '暫存失敗，請稍後再試';
        console.error('[MDO] updateDraft failed:', err);
        toast.error(msg);
        return;
      }
    }
  ...
  const handleRevertToV = async () => {
    const now = new Date();
    const pad = (n: number) => String(n).padStart(2, '0');
    const ts = `${now.getFullYear()}/${pad(now.getMonth() + 1)}/${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}`;
    const revertReason = `缺少欄位：${missingFields.join('、')}`;
    ...
    if (typeof order.id === 'string' && order.id.includes('-')) {
      try {
        ...
        const e = err as { body?: { error?: { message?: string } } };
        const msg = e?.body?.error?.message ?? '退回失敗，請稍後再試';
        console.error('[MDO] revertSampleOrder failed:', err);
        toast.error(msg);
        return;
      }
    }
      ...
      {showHistory && (
        <OrderHistory
          ...
          entries={mdoHistory.map(h => ({
            date: utcToLocalDisplay(h.changed_at),
            event: MDO_EVENT_LABEL[h.change_type] ?? h.change_type,
            operator: h.changed_by ?? '',
            remark: h.change_type === 'CANCEL'
              ? (h.cancel_reason ?? '')
              : h.change_type === 'REVERT'
                ? (h.revert_reason ?? '')
                : (h.change_reason ?? ''),
          }))}
          titleLabel="索樣單歷程"
